...
let g:factorize_java_identifier = s:start_chars . s:search_chars . '*'
...
function! s:find_tags(temp_file,search_string,append)
...
function! s:getCurrentTag()
    ...
    let a:taglines = readfile(&tags)
    let a:class_file = expand('%:p')
    let a:index = -1
    let a:max_line = 0

    while a:index < len(a:taglines) - 1
        let a:index += 1
        if match(a:taglines[a:index],a:class_file) == -1
            continue
        endif

        let a:tag = split(a:taglines[a:index],'\t')
        let a:search = strpart(a:tag[2],0,strridx(a:tag[2],';'))
        let a:search = strpart(a:search,1,len(a:search)-2)

        let a:line = search(a:search,'bn')

        if a:line > a:max_line && a:line <= line('.')
            echo a:search
            let a:max_line = a:line
        endif
...
function! s:updateLocalFiles(class_name,new_name,paren,...)
    ...
    if a:method_name == ""
        call system('for f in $(ls); do if [  -f "$f" ]; then sed -i "s/\<' . a:class_name . '\>' . a:paren . '/' . a:new_name . a:paren . '/g" $f; fi done')
    else
...
function! s:updatePackageFiles(class_file,new_name,paren,...)
    ...
    call s:find_tags(a:temp_file, a:import_search, 'no')
    call s:find_tags(a:temp_file, a:import_all_search, 'yes')
...
function! factorize#refactorClass(new_name)
...
function! factorize#refactorStatic(old_name,new_name,is_method)
    ...
    "let a:static_name = s:getCurrentTag()
...
function! s:getClosingBracket()
    ...
    while a:mark >= 0
        ...
        if a:oline < a:cline || (a:oline == a:cline && a:ocol < a:ccol)
            call s:goto(a:oline,a:ocol)
            let a:mark += 1
        else
            call s:goto(a:cline,a:ccol)
            let a:mark -= 1
        endif
    ...
    call s:goto(a:line,a:col)
    return [a:pline,a:pcol]

endfunction

function! factorize#refactorNonStatic(old_name,new_name,is_method,is_local) abort

    let a:class_name = expand('%:t:r')

    if a:is_local == 1
        echo a:old_name . ' is local'
...
function! factorize#getCurrentTag()
    echo s:getCurrentTag()
endfunction
