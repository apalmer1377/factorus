...
let g:collection_identifier = '\(\[\]\|<[<>,.[:space:]' . s:search_chars . ']*>\)'
let s:access_query = '\(public\_s*\|private\_s*\|protected\_s*\)\=\(static\_s*\)\=\(final\_s*\)\='

let g:factorus_java_identifier = '[' . s:start_chars . '][' . s:search_chars . ']*'
let g:remainder_query = '\_[,<>.[:space:]' . s:search_chars . ']*'

let g:factorus_tag_query = '^\s*' . s:access_query . '\(class\_s*\|enum\_s*\|' . g:factorus_java_identifier . g:collection_identifier . '\=\_s\+' . g:factorus_java_identifier . g:collection_identifier . '\=' . g:remainder_query . '(\)'
...
function! s:getNextDec(class_name,...)
    ...
    let a:get_variable = '^[^/*]*\([,([:space:]]' . a:class_name . g:collection_identifier . '\=\s*\)\(' . a:ident . '\)[:;=,[:space:])].*'
...
function! s:updatePackageFiles(class_file,new_name,paren,...)
    ...
    if a:method_name != ""
        call s:updateSubClassFiles(a:class_file,a:temp_file,a:method_name,a:new_name,a:paren,a:is_static)
    endif
...
function! s:updateSubClassFiles(class_file,temp_file,old_name,new_name,paren,is_static)
    ...
    let a:class_name = substitute(a:class_file,'\(.*\/\)\=\(.*\)\.java','\2','')
    let a:search = '\(implements\|extends\)[[:space:]]\+\<' . a:class_name . '\>'
...
function! factorus#refactorClass(new_name) abort
    ...
    call s:updatePackageFiles(expand('%:t:h'),a:new_name,'')
...
function! factorus#refactorField(new_name) abort
    let a:search = '\_s*' . s:access_query . '\(' . g:factorus_java_identifier . g:collection_identifier . '\=\)\_s*\(' . g:factorus_java_identifier . '\)\_s*[;=].*'
...
function! s:updateDeclarations(method_name,new_name)
    ...
    let a:subs = readfile(a:temp_file)
    for file in a:subs
        execute 'silent edit ' . file
        call s:updateDeclarations(a:method_name,a:new_name)
        bdelete
    endfor
...
function! factorus#encapsulateField() abort
    let a:search = '\s*' . s:access_query . '\(' . g:factorus_java_identifier . g:collection_identifier . '\=\)\_s*\(' . g:factorus_java_identifier . '\)\_s*[;=]'
