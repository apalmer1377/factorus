...
let g:factorus_method_identifier = '\(public\s\|private\s\|protected\s\)\=\s*\(static\s\)\=\s*' . g:factorus_java_identifier . '\(<' . s:collection_identifier . '>\)\=\s\s*' . g:factorus_java_identifier . '\(<' . s:collection_identifier . '>\)\=\s*([^;]*$'
let g:factorus_class_enum_identifier = '\(public\s\|private\s\|protected\s\)\=\s*\(static\s\)\=\s*.*\(class\|enum\)\s\+' . s:collection_identifier . '\s\+.*[\s{]$'
...
let g:factorus_tag_query = '\(' . g:factorus_method_identifier . '\|' . g:factorus_class_enum_identifier . '\)'
" Functions {{{1
...
function! s:getClosingBracket()

    let [a:line,a:col] = [line('.'),col('.')]
    let [a:pline,a:pcol] = [line('.'),col('.')]
    let [a:oline,a:ocol] = [line('.'),col('.')]
    let [a:cline,a:ccol] = [line('.'),col('.')]
    ...
    while a:mark >= 0
        "call search('{')
        "let [a:oline,a:ocol] = [line('.'),col('.')]
        "call cursor(a:pline,a:pcol)

        let a:open = s:getNext('{','')
        let a:close = s:getNext('}','')

        "call search('}')
        "let [a:cline,a:ccol] = [line('.'),col('.')]
        "call cursor(a:pline,a:pcol)

        if a:oline < a:cline || (a:oline == a:cline && a:ocol < a:ccol)
...
function! s:getCurrentTag()
    ...
    let a:is_valid = 0
    while a:is_valid == 0
        let a:func = search(g:factorus_tag_query,'b')
        if a:func > a:line
            break
        endif

        let a:is_valid = s:isValidTag(a:func)
        continue
...
function! s:getClassTag()
    let [a:line,a:col] = [line('.'),col('.')]
    call cursor(1,1)
    let a:class_tag = search(g:factorus_class_enum_identifier,'n')
    call cursor(a:line,a:col)
    return a:class_tag
endfunction
...
function! s:updateLocalFiles(class_name,new_name,paren,...)
    ...
    if a:method_name == ""
        call system('for f in $(ls); do if [  -f "$f" ] && [ "$f" != "' . expand('%:t') . '" ]; then sed -i "s/\<' . a:class_name . '\>' . a:paren . '/' . a:new_name . a:paren . '/g" $f; fi done')
    else
        echo a:method_name . ' is a method'
    endif
...
function! s:updatePackageFiles(class_file,new_name,paren,...)
    ...
    if a:method_name == ""
        call system('for line in $(cat ' . a:temp_file . '); do if [ -f "$line" ]; then sed -i "s/\<' . a:class_name . '\>/' . a:new_name . '/g" $line; fi done')
    elseif a:is_static == 1
        let a:static_name = a:class_name . '\\.' . a:method_name
        let a:new_static = a:class_name . '.' . a:new_name
        call system('for line in $(cat ' . a:temp_file . '); do if [ -f "$line" ]; then sed -i "s/\<' . a:static_name . '\>' . a:paren . '/' . a:new_static.a:paren . '/g" $line; fi done')
    else
        echo a:method_name . ' is a non-static method'
    endif
...
function! factorus#refactorStatic(old_name,new_name,is_method)
...
function! s:getNextDec(class_name)
    let a:variable_query = a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*' . g:factorus_java_identifier . '\s*[;=]'
    let a:get_variable = '\(.*' . a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*\)\(' . g:factorus_java_identifier . '\)\(\s*[;=].*\)'

    let a:line_n = search(a:variable_query,'nW')
    if a:line_n > 0
        let a:var = substitute(getline(a:line_n),a:get_variable,'\3','')
        return [a:line_n,a:var]
    endif
...
function! s:jumpToNext(vars,next)

    let a:line = line('.')
    let a:min_line = next[0] > 0 ? next[0] : line('$')
    for var in vars
        let a:match = search(var[0],'W') 
    endfor


endfunction

function! factorus#updateMethodFile(file_name,class_name,method_name,new_name)
    let a:variable_query = a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*' . g:factorus_java_identifier . '\s*[;=]'
    let a:get_variable = '\(.*' . a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*\)\(' . g:factorus_java_identifier . '\)\(\s*[;=].*\)'
    let a:vars = []
    ...
    let a:line_n = search(a:variable_query,'W')
    while a:line > 0
        let a:line = getline(a:line_n)
        let a:var = substitute(a:line,a:get_variable,'\3','')
        echo a:var
        call add( a:vars , [a:var , s:getClosingBracket()] )
...
function! factorus#test(class_name)
    let a:line = getline('.')
    let a:q = '\(.*' . a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*\)\(' . g:factorus_java_identifier . '\)\(\s*[;=].*\)'
    echo match(a:line,a:q)
    let a:var = substitute(a:line, '\(.*' . a:class_name . '\(<' . s:collection_identifier . '>\)\=\s*\)\(' . g:factorus_java_identifier . '\)\(\s*[;=].*\)','\3','')
    echo a:var
    echo a:line
endfunction 


function! factorus#refactorNonStatic(old_name,new_name,is_method,is_local) abort
    ...
    if a:is_method == 1
        ...
        call s:updateMethodLocal(a:old_name,a:new_name)
        call s:updateMethodPackage(a:old_name,a:new_name)

    elseif a:is_local == 1
        let a:next = search('\<' . a:old_name . '\>')
        let a:line = line('.')
        let a:col = col('.')
        let a:closing = s:getClosingBracket()

        while a:line < a:closing[0] || (a:line == a:closing[0] && a:col < a:closing[1])

            execute 's/\<' . a:old_name . '\>/' . a:new_name . '/'

            let a:next = search('\<' . a:old_name . '\>')
            if a:next == 0 || (a:closing[0] < line('.') || (a:closing[0] == line('.') && a:closing[1] < col('.')))
                break
            endif
...
function! factorus#refactorMethod(new_name) abort
    ...
    if a:is_static >= 0
        call factorus#refactorStatic(a:method_name,a:new_name,1)
    else
        call factorus#refactorNonStatic(a:method_name,a:new_name,1,0)
    endif
