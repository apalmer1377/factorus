...
function! s:getTypeDefs(name,...)
    ...
    let l:search = '\<typedef\>' . l:type . a:name . '\_s*\<\(' . s:c_identifier . '\)\>'
    try
...
function! s:getNextDec()
    let l:get_variable = '^\s*\(' . s:modifier_query . '\|for\s*(\)\s*\(' . s:c_type . '\_s*' . 
                \ s:collection_identifier . '\)\s*\(\<' . s:c_identifier . '\>[^=;]*\)[;=].*'
    
    let l:alt_get = '^\s*' . s:modifier_query . '\s*\(' . s:c_type . '\_s*' . 
                \ s:collection_identifier . '\)\s*\(\<' . s:c_identifier . '\>[^=;]*\)[=;].*'
    ...
    if s:isBefore([l:line,l:col],l:match) == 1
        ...
        call map(l:fline,{n,var -> substitute(var,'^\<\(' . s:c_identifier . '\)\>.*','\1','')})
...
function! s:getFunctionDecs()
    let l:query = '^\s*' . s:modifier_query . '\s*\(' .  s:c_type . '\_s*' . s:collection_identifier . '\)\_s*\(' . s:c_identifier . '\)\_s*\([;(]\).*'
    ...
    try
        ...
        for g in l:greps
            ...
            if match(l:fname,s:c_keywords) >= 0
                continue
            endif
...
function! s:getFuncDec(func)
    let l:orig = [line('.'),col('.')]
    call cursor(1,1)
    let l:search = s:no_comment . s:modifier_query . '\(' . s:c_type . '\_s*' . s:collection_identifier . '\)\_s\+\<' . a:func . '\(\<\|\>\|)\|\s\).*'
...
function! s:getVarDec(var)
    ...
    while s:isQuoted(l:res,getline(l:pos)) == 1 || s:isCommented() == 1 || match(l:res,s:c_keywords) >= 0
        if l:pos == 0
            return ''
        endif
    ...
    if l:is_local == 1
        ...
        if match(getline('.'),'\<typedef\>') >= 0
            ...
            if match(getline('.'),'\<\(' . s:c_identifier . '\)\>') >= 0
                call add(l:type_defs,substitute(getline('.'),'.*\<\(' . s:c_identifier . '\)\>.*','\1',''))
            endif
            call cursor(l:prev)
        endif
        ...
        " union.
        let l:search = '^\s*\(\<typedef\>\)\=\_s*\<\(struct\|union\)\>\_s*\(' . s:c_identifier . '\)\=\_s*{\=.*'
...
function! s:renameMacro(new_name) abort
    ...
    let l:search = '^#define\s\+\<\(' . s:c_identifier . '\)\>.*'
...
function! s:renameMethod(new_name) abort
    ...
    " the same name.
    let l:method_name = matchstr(getline('.'),'\<' . s:c_identifier . '\>\s*(')
...
function! s:renameType(new_name) abort
    ...
    let l:search = '^.*\<\(enum\|struct\|union\)\>\s*\(\<' . s:c_identifier . '\>\)\s*\({\|\<' . s:c_identifier . '\>\_s*;\).*'
...
function! c#factorus#addParam(param_name,param_type,...) abort
    ...
    try
        ...
        let [l:type,l:name,l:params] = split(substitute(join(getline(line('.'),l:next[0])),'^.*\<\(' . s:c_type . 
